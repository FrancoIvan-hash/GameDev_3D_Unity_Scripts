...
public class PlayerTargetingState : PlayerBaseState
{
    ...
    {
        // Time.time - PreviousDodgeTime: how much time has passed since our last dodge
        // if that is less than our cooldown, then we can't dodge yet
        if (Time.time - stateMachine.PreviousDodgeTime < stateMachine.DodgeCooldown) { return; }

        stateMachine.SetDodgeTime(Time.time);

        // the way we want to move when we hit the dodging button
    ...
    private Vector3 CalculateMovement(float deltaTime)
    {
        ...
        {
            // transform.right * dodgingDirectionInput.x gives us the direction
            // DodgeLength / DodgeDuration gives us the duration
